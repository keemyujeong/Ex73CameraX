...
import com.kyjsoft.ex73camerax.R
...
class MainActivity2 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<View>(R.id.fab_image_capture).setOnClickListener { view: View? -> clickCapture() } // 카메라가 액티비티 생명주기랑 연결된 후 버튼 누르면 캡쳐본 뜨게

        // 동적 퍼미션들
        var permissions: Array<String?>? =
            null //28 버전이상인거랑 낮은 거랑 구분 할라고 미리 안주는 거임.-> 28 이상은 write 안줘도 됨.
        permissions = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) arrayOf(
    ...
    fun startPreview() {
        ...
        val listenableFuture = ProcessCameraProvider.getInstance(this) // 연결결과를 들을 수 있는 녀석
...
//        }, ContextCompat.getMainExecutor(this));
    ...
    fun clickCapture() {
        ...
        val filename =
            SimpleDateFormat("yyyyMMddHHmmss").format(System.currentTimeMillis()) // 자바 방법
        // 사진 정보가 저장된 MediaStore(미디어 저장소)의 DB에 넣을 값들(1개의 record)를 가진 객체
        val contentValues = ContentValues()
        contentValues.put(
            MediaStore.MediaColumns.DISPLAY_NAME,
            filename
        ) // db 특정 하나의 칸에 filename이라고 값 넣기
        ...
        imageCapture!!.takePicture(
            ...
            object : ImageCapture.OnImageSavedCallback {
                override fun onImageSaved(outputFileResults: ImageCapture.OutputFileResults) {
                    Toast.makeText(this@MainActivity2, "촬영 성공", Toast.LENGTH_SHORT).show()

                    // 저장될 파일의 uri
                    val uri = outputFileResults.savedUri
                    Glide.with(this@MainActivity2).load(uri).into(iv!!)
                }

                override fun onError(exception: ImageCaptureException) {
                    Toast.makeText(
                        this@MainActivity2,
                        "캡쳐 실패 :" + exception.message,
                        Toast.LENGTH_SHORT
                    ).show()
                }
            })
